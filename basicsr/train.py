...
def main():
    ...
    resume_state = None
    if opt.get("auto_resume", False) and len(states) > 0:
    ...
    best_psnr_models = []  # To track top 2 PSNR models: list of tuples (psnr, current_iter)
    ...
    mini_accumulation_steps = opt["train"].get(
        "mini_accumulation_steps"
    )  # Get from config
            ...
            if total_fetches % acc_steps == 0:
                ...
                if current_iter % opt["logger"]["print_freq"] == 0:
                    ...
                    if torch.cuda.is_available():
                        log_vars["max_mem"] = torch.cuda.max_memory_allocated() / (
                            1024 * 1024 * 1024
                        )
                        log_vars["max_res"] = torch.cuda.max_memory_reserved() / (
                            1024 * 1024 * 1024
                        )
                    msg_logger(log_vars)
                ...
                if opt.get("val") is not None and (
                    current_iter % int(opt["val"]["val_freq"]) == 0
                ):
                    ...
                    # Track and save top 2 PSNR models
                    psnr_key = next(
                        (k for k in model.metric_results.keys() if "psnr" in k.lower()),
                        None,
                    )
                    if psnr_key:
                        current_psnr = model.metric_results[psnr_key]
                        best_psnr_models.append((current_psnr, current_iter))
                        best_psnr_models.sort(key=lambda x: x[0], reverse=True)

                        keep_iters = [x[1] for x in best_psnr_models[:2]]

                        if current_iter in keep_iters:
                            logger.info(
                                f"New top psnr {current_psnr:.4f} at iter {current_iter}. Saving model .pth."
                            )

                            save_name = f"{current_iter}_{current_psnr:.4f}"
                            if getattr(model, "ema_decay", 0) > 0:
                                model.save_network(
                                ...
                                model.save_network(model.net_g, "net_g", save_name)

                        # Delete older .pth files not in top 2
                        import glob
                        import os

                        while len(best_psnr_models) > 2:
                            old_psnr, old_iter = best_psnr_models.pop()
                            model_folder = opt["path"]["models"]

                            for pattern in [f"*_{old_iter}.pth", f"*_{old_iter}_*.pth"]:
                                for pths in glob.glob(
                                    os.path.join(model_folder, pattern)
                                ):
                                    try:
                                        os.remove(pths)
                                    except Exception:
